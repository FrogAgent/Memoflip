...
public class Vegetables_16_CardsGame : MonoBehaviour
{
    ...
    private void CheckForGameOver()
    {
        if (matchesFound == cards.Count / 2)
        {
            ...
            if (!isEasyMode)
            {
                ...
                if (difficulty == "HARD")
                {
                    currentScore += Mathf.RoundToInt(timeLeft * 800);
                }
                else
                {
                    currentScore += Mathf.RoundToInt(timeLeft * 40);
                }
    ...
    private void SaveScore(int score)
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            scores.Add(PlayerPrefs.GetInt("Vegetables_16_Score_" + i, 0));
        }
        ...
        {
            PlayerPrefs.SetInt("Vegetables_16_Score_" + i, scores[i]);
        }
