...
public class Vegetables_24_CardsGame : MonoBehaviour
{
    ...
    private void CheckForGameOver()
    {
        if (matchesFound == cards.Count / 2)
        {
            ...
            if (!isEasyMode)
            {
                ...
                if (difficulty == "HARD")
                {
                    currentScore += Mathf.RoundToInt(timeLeft * 2000);
                }
                else
                {
                    currentScore += Mathf.RoundToInt(timeLeft * 60);
                }
    ...
    private void SaveScore(int score)
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            scores.Add(PlayerPrefs.GetInt("Vegetables_24_Score_" + i, 0));
        }
        ...
        {
            PlayerPrefs.SetInt("Vegetables_24_Score_" + i, scores[i]);
        }
